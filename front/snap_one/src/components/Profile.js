...
function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchUserPhotos = async () => {
      try {
        const response = await fetch(`/api/photos/user/${userId}?api=${apiKey}&uname=${username}`);
        const data = await response.json();
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedPhotosResponse = await fetch(`/api/photos/user/${userId}?api=${apiKey}&uname=${username}`);
